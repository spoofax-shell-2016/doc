...
Spoofax come with a textual format called ATerms~\cite{Brand00}. As an example,
a term representation of the arithmetic expression $2 + 2 - 4$ is shown in
\cref{lst:aterm-example}.
...
compilation to another high-level language.
...
equation~(\ref{eq:formal-spec}), which is explained
later. \Cref{lst:dynsem-rule-syntax} shows the syntax of a reduction rule as
specified in DynSem.
